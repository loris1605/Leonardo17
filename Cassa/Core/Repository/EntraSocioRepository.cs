...
namespace Cassa.Core.Repository
{
    ...
    public class EntraSocioRepository(IEntraSocioDbContext ctx) : BaseRepository<EntraSocioDbContext, Scheda>, IEntraSocioRepository
    {
        ...
        public async Task<int> AddNewScheda(EntraSocioDTO dto, EntraIngressiDTO edto, CancellationToken ctk = default)

        {
            var dataOra = DateTime.Now;
            ...
            var scheda = new Scheda
            {
                ...
                Blocco = dto.Blocco,
                Note = dto.Note,
                SchedeConto = new List<SchedaConto>
                {
                    new() {
                        // Imposta le proprietà di SchedaConto se necessario
                        DescSettore = "Ingresso",
                        DescPostazione = "Cassa",
                        VoiceDesc = edto.NomeTariffa,
                        VoicePrice = edto.PrezzoTariffa,
                        Pagato = true,
                        Note = "Ingresso registrato",
                        DataOra = dataOra
                    }
                }
            };
        ...
        //public async Task<int> AddNewSchedaConto(EntraSocioDTO dto, CancellationToken ctk = default)
        //{
        //    var schedaConto = new SchedaConto
        //    {

        //    };

        //    return -1; // Placeholder per l'implementazione futura
        //}
