...
namespace Cassa.ViewModels.Map
{
    public class CassaSchedaMap : BindableMap
    {
        ...
        public string NatoilFormattato
        {
            get
            {
                // Esempio: Se Natoil è un intero tipo 19900512 (AAAAMMGG)
                string dataStr = Natoil.ToString();
                if (dataStr.Length == 8 &&
                    DateTime.TryParseExact(dataStr, "yyyyMMdd", null, System.Globalization.DateTimeStyles.None, out DateTime dt))
                {
        ...
        // Ora osservabile: ObservableCollection aggiorna la DataGrid quando gli elementi cambiano
        private ObservableCollection<CassaSchedaContoMap> _conti = new();
        public ObservableCollection<CassaSchedaContoMap> Conti
        {
            get => _conti;
            set => this.RaiseAndSetIfChanged(ref _conti, value);
        }
