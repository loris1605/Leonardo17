...
namespace Menu.ViewModels
{
    ...
    // Creiamo osservabili "resilienti": osservano la proprietà del comando (anche se null)
    // e poi usano Switch() per sottoscrivere l'IsExecuting reale quando il comando viene creato.
    Observable.CombineLatest(
        ...
        // osservabili "dinamiche" per i comandi derivati
        this.WhenAnyValue(x => x.CassaPostazioneCommand)
            .Select(cmd => cmd != null ? cmd.IsExecuting : Observable.Return(false))
            .Switch()
            .StartWith(false),
        this.WhenAnyValue(x => x.LogoutCommand)
            .Select(cmd => cmd != null ? cmd.IsExecuting : Observable.Return(false))
            .Switch()
            .StartWith(false),
        this.WhenAnyValue(x => x.ConnectionCommand)
            .Select(cmd => cmd != null ? cmd.IsExecuting : Observable.Return(false))
            .Switch()
            .StartWith(false),
        this.WhenAnyValue(x => x.ConfigurazioneCommand)
            .Select(cmd => cmd != null ? cmd.IsExecuting : Observable.Return(false))
            .Switch()
            .StartWith(false),
        this.WhenAnyValue(x => x.SociCommand)
            ...
            .Switch()
            .StartWith(false),
        this.WhenAnyValue(x => x.ApriGiornataCommand)
            .Select(cmd => cmd != null ? cmd.IsExecuting : Observable.Return(false))
            .Switch()
            .StartWith(false),
        this.WhenAnyValue(x => x.ChiudiGiornataCommand)
            .Select(cmd => cmd != null ? cmd.IsExecuting : Observable.Return(false))
            .Switch()
            .StartWith(false),
        ...
        public MenuViewModel(IMenuRepository Repository) : base(null)
        {
            ...
            var canChiudiFinal = this.WhenAnyValue(x => x.ApriGiornataEnabled)
                .Select(x => !x)
                .ObserveOn(RxSchedulers.MainThreadScheduler);
        ...
        protected override async Task OnLoading()
        {
            ...
            try
            {
                // 1) Esegui work I/O in background (non toccare la UI qui)
                ...
                var esisteGiornata = await Q.EsisteGiornataAperta(Token).ConfigureAwait(false);
                var postazioniCount = GlobalValuesC.MySetting.POSTAZIONI?.Count ?? 0;
                var operatoreName = "Operatore : " + GlobalValuesC.MySetting.NOMEOPERATORE;

                // 2) Marshalla tutti i set delle proprietà che notificano la UI sul MainThread
                RxSchedulers.MainThreadScheduler.Schedule(() =>
                {
                    try
                    {
                        AttivaPermessi(); // se imposta proprietà UI-safe
                        CassaPostazioniDataSource = mapped;
                        ApriGiornataEnabled = !esisteGiornata;
                    ...
                    {
                        Debug.WriteLine($"Errore durante l'aggiornamento UI in OnLoading: {exUi}");
                        CassaPostazioniDataSource = new List<MenuPostazioneMap>();
                    }
                });
            }
            catch (OperationCanceledException)
            {
                Debug.WriteLine("OnLoading annullato.");
                RxSchedulers.MainThreadScheduler.Schedule(() => CassaPostazioniDataSource = new List<MenuPostazioneMap>());
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Errore OnLoading MenuViewModel: {ex}");
                RxSchedulers.MainThreadScheduler.Schedule(() => CassaPostazioniDataSource = new List<MenuPostazioneMap>());
            }
        }
    ...
    public partial class MenuViewModel
    {
        ...
        private async Task ExecuteChiudiGiornata()
        {
            //bool result = await Task.Run(() => Q.CloseGiornata(Token), Token);
            //if (result)
            //{
            //    ApriGiornataEnabled = true;
            //}
            await Task.CompletedTask;
        }
    }
